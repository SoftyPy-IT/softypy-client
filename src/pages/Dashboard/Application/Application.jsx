...
import { useDeleteApplicationMutation, useGetApplicationQuery } from "../../../redux/features/employee/employeeApi";
...
if(isError){
  return <p>Something went to wrong.</p>
    ...
    return (
        ...
        <h3 className="text-xl text-center mb-3 font-bold">Total Application {application?.length}</h3>
        <div className="overflow-x-auto ">
          <table className="table ">
            ...
            <tbody>
             {
              application?.map((order, i )=> <tr key={order._id}>
                    <td>{i}</td>
                   <td>{order.name}</td>
                   <td>{order.email}</td>
                   <td>{order.phone}</td>
